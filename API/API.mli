...
sig
  ...
  val body_of_constant :
    Opaqueproof.opaquetab -> Declarations.constant_body -> Constr.t option
end
...
sig
  ...
  val universes_of_inductive : Declarations.mutual_inductive_body -> Univ.universe_set
...
sig
  ...
  val body_of_constant_body : Declarations.constant_body -> Constr.t option
  val body_of_constant : Names.Constant.t -> Constr.t option
...
sig
  ...
  val unsafe_constr_of_global : Globnames.global_reference -> Constr.t Univ.in_universe_context
...
sig
  ...
  | VernacAssumption of (Decl_kinds.locality option * Decl_kinds.assumption_object_kind) *
      Declaremods.inline * (plident list * Constrexpr.constr_expr) with_coercion list
...
sig
  ...
  type ltac_var_map = Glob_ops.ltac_var_map = {
...
sig
  ...
  val instantiate_pattern : Environ.env ->
    Evd.evar_map -> Glob_ops.extended_patvar_map ->
...
sig
  ...
  val extended_matches :
    ...
    EConstr.constr -> bound_ident_map * Glob_ops.extended_patvar_map
  exception PatternMatchingFailure
  type matching_result =
    { m_sub : bound_ident_map * Glob_ops.patvar_map;
  ...
  val matches : Environ.env -> Evd.evar_map -> Pattern.constr_pattern -> EConstr.constr -> Glob_ops.patvar_map
end
...
sig
  ...
  type glob_constr_ltac_closure = Glob_ops.ltac_var_map * Glob_term.glob_constr

  val understand_ltac : inference_flags ->
                        Environ.env -> Evd.evar_map -> Glob_ops.ltac_var_map ->
  ...
  val type_uconstr :
    ...
    Geninterp.interp_sign -> Glob_ops.closed_glob_constr -> EConstr.constr Tactypes.delayed_open
  ...
  val register_constr_interp0 :
    ...
    (Glob_ops.unbound_ltac_var_map -> Environ.env -> Evd.evar_map -> EConstr.types -> 'g -> EConstr.constr * Evd.evar_map) -> unit
  val all_and_fail_flags : inference_flags
  val ise_pretype_gen :
    inference_flags -> Environ.env -> Evd.evar_map ->
    Glob_ops.ltac_var_map -> typing_constraint -> Glob_term.glob_constr -> Evd.evar_map * EConstr.constr
end
...
sig
  ...
  val wit_uconstr : (Constrexpr.constr_expr , Tactypes.glob_constr_and_expr, Glob_ops.closed_glob_constr) Genarg.genarg_type
  val wit_red_expr :
...
sig
  ...
  val pr_closed_glob : Glob_ops.closed_glob_constr -> Pp.std_ppcmds
  ...
  val pr_lconstr_under_binders : Glob_ops.constr_under_binders -> Pp.std_ppcmds
  val pr_lconstr_under_binders_env : Environ.env -> Evd.evar_map -> Glob_ops.constr_under_binders -> Pp.std_ppcmds

  val pr_constr_under_binders_env : Environ.env -> Evd.evar_map -> Glob_ops.constr_under_binders -> Pp.std_ppcmds
  val pr_closed_glob_env : Environ.env -> Evd.evar_map -> Glob_ops.closed_glob_constr -> Pp.std_ppcmds
...
sig
  ...
  type change_arg = Glob_ops.patvar_map -> Evd.evar_map -> Evd.evar_map * EConstr.constr
