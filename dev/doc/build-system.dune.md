...
build the full Coq distribution in developer mode; you can find the
build under `_build/install/default/`. Note that no call to
`./configure` is needed. If you would like to build a "release"
version to install, read below.
...
`dune build $target` is another common form; more helper targets are
available in `Makefile.dune`, `make -f Makefile.dune` will display
help.
...
targets in one or the other directory.
...
Dune does al it will also generate an `.install`
file so files can be properly installed by package managers.
