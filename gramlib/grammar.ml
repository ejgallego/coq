...
let skip_if_empty bp p strm =
  ...
  else raise (Stream.Failure "skip_if_empty")
  ...
  try p1 gstate strm__ with
    ...
    with
      ...
      try
        ...
        skip_if_empty bp (fun (strm__ : _ LStream.t) ->
            let msg = Format.asprintf "@[parser_cont: discard failed@\n [action 1: %s]@\n action 2: %s@]" err_msg_1 err_msg_2 in
            raise (Stream.Failure msg))
          strm__
      with
        (* XXX: Add some debug information? *)
        Stream.Failure msg_err_1 ->
  ...
  let rec loop : type tr lt r. int -> lt pattern -> (s, tr, r) ty_tree -> GState.t -> lt -> r parser_t =
    ...
    | Node (_, {node = Stoken tok; son = son; brother = bro}) ->
       ...
       fun gstate last_a strm ->
        (match (try Some (tematch (LStream.peek_nth gstate.kwstate n strm))
               (* XXX: Add some debug information? *)
    ...
    | _ ->
       ...
       fun gstate last_a strm ->
         ...
         match
           try Some (ps gstate strm) with
             (* XXX: Add some debug information? *)
             Stream.Failure _ ->
             (* Tolerance: retry w/o granting the level constraint (see recover) *)
             try Some (parser_of_tree entry nlevn alevn (top_tree entry tree) gstate strm)
             with Stream.Failure _ ->
               (* XXX: Add some debug information? *)
               None
         with
...
let rec continue_parser_of_levels entry clevn =
  function
  | [] -> (fun _gstate levn bp a (strm__ : _ LStream.t) ->
...
let make_continue_parser_of_entry entry desc =
  match desc with
  | Dlevels [] ->
    (fun _ _ _ _ (_ : _ LStream.t) ->
       raise (Stream.Failure "continue_parser_of_entry: empty list"))
  | Dlevels elev ->
    let p = lazy (continue_parser_of_levels entry 0 elev) in
    (fun gstate levn bp a (strm__ : _ LStream.t) ->
       try Lazy.force p gstate levn bp a strm__
       with Stream.Failure _ -> a)
  | Dparser p -> fun gstate levn bp a (strm__ : _ LStream.t) ->
    raise (Stream.Failure "continue_parser_of_entry: parser entry")
...
module Entry = struct
  ...
  let empty_entry_val e = {
    ...
    econtinue = (fun _ _ _ _ (strm__ : _ LStream.t) ->
        raise (Stream.Failure "econtinue: empty_entry_val"));
  }
  ...
  let of_parser_val e { parser_fun = p } = {
    ...
    econtinue = (fun _ _ _ _ (strm__ : _ LStream.t) ->
        raise (Stream.Failure "econtinue: of_parser_val"));
    edesc = Dparser p;
  }
